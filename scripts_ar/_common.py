"""Common setup for AR funnel test scripts.

Usage: python scripts_ar/stage_N.py  (from project root or any directory)
"""
...
async def send_stage(stage: int) -> None:
    """Set funnel_stage in DB and send the AR funnel message to test user."""
    configure_logging()
    ...
    logger.info("stage_set", stage=stage, language="ar", chat_id=CHAT_ID)

    # Get and send funnel message
    msg = get_funnel_message(stage, "ar")
    if msg is None:
        logger.error("no_message_for_stage", stage=stage, language="ar")
    else:
...
async def reset_user() -> None:
    """Reset test user to initial AR funnel state."""
    configure_logging()
    logger = structlog.get_logger()
    ...
    logger.info("user_reset", chat_id=CHAT_ID, language="ar")
