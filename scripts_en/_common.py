"""Common setup for EN funnel test scripts.

Usage: python scripts_en/stage_N.py  (from project root or any directory)
"""
...
async def send_stage(stage: int) -> None:
    """Set funnel_stage in DB and send the EN funnel message to test user."""
    configure_logging()
    ...
    logger.info("stage_set", stage=stage, language="en", chat_id=CHAT_ID)

    # Get and send funnel message
    msg = get_funnel_message(stage, "en")
    if msg is None:
        logger.error("no_message_for_stage", stage=stage, language="en")
    else:
...
async def reset_user() -> None:
    """Reset test user to initial EN funnel state."""
    configure_logging()
    logger = structlog.get_logger()
    ...
    logger.info("user_reset", chat_id=CHAT_ID, language="en")
